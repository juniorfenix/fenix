    ...
    <>
      ...
      <div className="mt-2">
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          <span>{done.size} de {MEAL_PREP_STEPS.length} concluídos</span>
          <span>{pct}%</span>
        </div>
            ...
            <li key={i}>
              <button
                ...
                className={`w-full rounded-xl border p-3 text-left transition ${
                  checked ? "border-primary/40 bg-primary/5" : "border-border bg-card/50 hover:border-primary/30"
                }`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <div className={`text-[15px] font-medium ${checked ? "line-through text-muted-foreground" : ""}`}>{s.title}</div>
                    <div className="text-[13px] text-foreground/65 mt-1 leading-relaxed">{s.desc}</div>
                  </div>
                </div>
              </button>
            </li>
    ...
    return (
      <>
        <DialogHeader>
          ...
          <DialogTitle className="font-display text-2xl tracking-tight">{selected.nome}</DialogTitle>
        </DialogHeader>
        <div className="mt-2 flex items-center gap-3 text-xs text-muted-foreground">
          <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {selected.tempo} min</span>
          <span>•</span>
        ...
        <div className="mt-4">
          <h4 className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground mb-3">Ingredientes</h4>
        ...
        <div className="mt-5">
          <h4 className="text-[11px] uppercase tracking-[0.2em] text-muted-foreground mb-3">Modo de preparo</h4>
        ...
        {CATEGORIAS.map((c) => (
          <button
            ...
            className={`text-xs px-3 py-1.5 rounded-full border transition ${
              cat === c ? "border-primary bg-primary text-primary-foreground" : "border-border text-muted-foreground hover:border-primary/40"
            }`}
          >
            {c}
          </button>
            ...
            >
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  ...
                  <div className="text-[12.5px] text-foreground/60 mt-1">{r.categoria} • {r.kcal} kcal</div>
                </div>
...
export function ProtocoloPlateau() {
      ...
      <>
        ...
        {recomendadas.length === 0 ? (
          <div className="mt-4 rounded-xl border border-border bg-card/50 p-4 text-sm">
            Nenhum gatilho clássico identificado. Antes de mexer no plano, audite 7 dias com tudo registrado (refeições, líquidos, treino e sono). 80% dos "plateaus" são erro de leitura, não adaptação real.
          </div>
        ) : (
          <div className="mt-4 space-y-3">
            <div className="text-xs uppercase tracking-widest text-muted-foreground">Estratégia recomendada</div>
            {recomendadas.map((o) => (
        ...
        <Button onClick={reset} variant="outline" className="mt-4 w-full">Refazer diagnóstico</Button>
      </>
    );
  }
  ...
  return (
    <>
      ...
      <div className="mt-2 text-xs text-muted-foreground">Pergunta {step + 1} de {PLATEAU_OPCOES.length}</div>
      <div className="mt-1 h-1.5 overflow-hidden rounded-full bg-secondary">
        <div className="h-full bg-gradient-ember transition-all" style={{ width: `${((step + 1) / PLATEAU_OPCOES.length) * 100}%` }} />
      </div>
      <p className="mt-5 font-display text-xl leading-snug tracking-tight text-foreground/95">{atual.pergunta}</p>
      <div className="mt-5 grid grid-cols-2 gap-3">
        <Button variant="outline" className="h-12" onClick={() => respond(false)}>Não</Button>
        <Button className="h-12 bg-gradient-ember text-primary-foreground" onClick={() => respond(true)}>Sim</Button>
      </div>
    </>
  );
}
