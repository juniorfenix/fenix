...
import { s as supabase } from "./router-DHEQdz8j.mjs";
...
const exerciciosQuery = queryOptions({
  queryKey: ["exercicios"],
  queryFn: async () => {
    const { data, error } = await supabase.from("exercicios").select("id,nome,grupo_muscular,nivel,equipamento,gif_url,video_url,tipo_midia").order("nome", { ascending: true });
    if (error) throw error;
    return data ?? [];
  },
  staleTime: 10 * 60 * 1e3
...
export {
  ...
  planosTreinoInstrutorQuery as n,
  planosAlimentaresInstrutorQuery as o,
  profileQuery as p,
  exerciciosQuery as q,
