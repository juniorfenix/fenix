...
import { profileQuery, badgesQuery, weightsQuery } from "@/lib/queries";
...
function Profile() {
  ...
  // Auto-unlock heuristic so visual state matches user expectation
  // (badges row in DB is the source of truth; this is a UI fallback).
    ...
    try {
      ...
      if (profile.current_weight && profile.height && profile.age && profile.gender) {
        daily_calorie_goal = calculateDailyCalories({
          ...
          goal: goal_weight < profile.current_weight ? "lose"
            : goal_weight > profile.current_weight ? "gain"
            : "maintain",
        });
      }
    ...
    <main className="mx-auto max-w-md px-5 pt-10 pb-8">
      ...
      <section className="mt-8">
        <div className="flex items-center gap-2 mb-3">
          ...
          <h2 className="text-xs uppercase tracking-widest text-muted-foreground">Histórico de peso</h2>
          {historico.length > 0 && (
        ...
        <div className="glass rounded-2xl divide-y divide-border/40 overflow-hidden">
          {historico.length === 0 ? (
            ...
            historico.slice(0, 10).map((w, idx) => {
              ...
              const label = d.toLocaleDateString("pt-BR", { day: "2-digit", month: "short", year: "2-digit" });
              return (
                <div key={`${w.logged_date}-${idx}`} className="flex items-center justify-between px-4 py-3">
        ...
        <DialogContent>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-1.5">
              ...
              />
              {errors.goal_weight && <p className="text-xs text-destructive">{errors.goal_weight}</p>}
            </div>
            <div className="space-y-1.5">
            ...
            <div className="rounded-xl border border-primary/20 bg-primary/5 px-3 py-2.5 text-xs">
              {previewCalories != null ? (
                <>
                  ...
                  {profile?.daily_calorie_goal && previewCalories !== profile.daily_calorie_goal && (
                    <span className="text-muted-foreground"> (era {profile.daily_calorie_goal})</span>
                  )}
                </>
              ) : (
                <span className="text-muted-foreground">Informe um peso válido para ver a nova meta.</span>
              )}
            </div>
            <Button
              onClick={saveEdit}
